...
const getMockAnalysis = (): AIAnalysisResult => ({
  summary:
    "You have shown remarkable consistency this week! Your hydration and reading habits are strong, though weekend routines could be tightened.",
  score: 82,
  tips: [
...
const MOCK_SUGGESTIONS: HabitSuggestion[] = [
  {
    ...
    reason: "Kickstart your circulation and energy.",
    icon: "🧘",
    color: "bg-orange-500",
    time: "07:00",
    difficulty: "Easy",
  },
  {
    ...
    reason: "Maximize focus for complex tasks.",
    icon: "🧠",
    color: "bg-indigo-500",
    time: "10:00",
    difficulty: "Hard",
  ...
  {
    title: "Gratitude Log",
    reason: "Improve mental well-being daily.",
    icon: "📓",
    color: "bg-pink-500",
    time: "21:30",
    difficulty: "Easy",
...
const getMockWeeklyReview = (): WeeklyReviewData => ({
  weekScore: 78,
  summary:
    "A solid week of performance. You crushed your targets on Tuesday and Wednesday but trailed off slightly on Saturday.",
  bestDay: "Tuesday",
        ...
        responseSchema: {
          ...
          items: {
            ...
            properties: {
              title: {
                type: Type.STRING,
                description: "Short title e.g. 'Read 10 pages'",
              },
              reason: {
                type: Type.STRING,
              ...
              color: {
                type: Type.STRING,
                description: "Tailwind class from provided list",
              },
...
export const analyzeWeekly = async (
  ...
  const weeklyLogs = last7Days
    .map((date) => {
      const dayLog = logs[date] || {};
      const completed = Object.values(dayLog).filter(
        (s) => s === "completed"
      ).length;
...
export const getPersonalizedMotivation = async (
  name: string,
  streak: number
): Promise<string> => {

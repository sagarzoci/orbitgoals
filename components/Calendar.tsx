...
const Calendar: React.FC<CalendarProps> = ({ 
  ...
  const generateDays = () => {
    ...
    for (let i = 0; i < startDay; i++) {
      // Shorter padding for mobile to save vertical space if needed, or keeping transparent
      days.push(<div key={`empty-${i}`} className="h-14 sm:h-24 md:h-32 bg-transparent" />);
    }
    ...
    for (let day = 1; day <= daysInMonth; day++) {
      ...
      days.push(
        <motion.div 
          ...
          className={`
            relative h-14 sm:h-24 md:h-32 rounded-lg sm:rounded-2xl border p-1 sm:p-3 cursor-pointer overflow-hidden group select-none
          ...
          `}
        >
            <div className={`absolute inset-0 bg-indigo-600 transition-opacity duration-500 pointer-events-none`} style={{ opacity: bgOpacity }} />
            
            {isToday && <div className="absolute inset-0 rounded-lg sm:rounded-2xl border-2 border-indigo-500/40 animate-pulse pointer-events-none" />}
            
            <div className="relative z-10 flex justify-between items-start">
                <span className={`
                  text-xs sm:text-sm font-bold w-5 h-5 sm:w-7 sm:h-7 flex items-center justify-center rounded-md sm:rounded-lg transition-colors
            ...
            {/* Mobile: Simple bar instead of dots if too small */}
            <div className="relative z-10 flex sm:hidden items-end h-full pt-1">
            ...
            {/* Desktop: Dots */}
            <div className="relative z-10 hidden sm:flex flex-wrap gap-0.5 sm:gap-1.5 content-end">
                {goals.slice(0, 8).map((g, i) => {
                  const status = dayLogs[g.id];
                  let dotClass = 'bg-slate-800 ring-1 ring-slate-700'; 
                  if (status === 'completed') dotClass = `${g.color} ring-0 shadow-[0_0_8px_rgba(255,255,255,0.3)]`;
                  else if (status === 'skipped') dotClass = 'bg-rose-500/20 ring-1 ring-rose-500/50';

                  const visibilityClass = i >= 4 ? 'hidden md:block' : ''; // Responsive dot hiding

                  return (
                    <div key={g.id} className={`w-1.5 h-1.5 sm:w-2.5 sm:h-2.5 rounded-full transition-all duration-300 ${dotClass} ${visibilityClass}`} />
                  );
                })}
            </div>
        </motion.div>
    ...
    <div className="bg-slate-900/80 backdrop-blur-xl rounded-3xl border border-slate-800 p-4 sm:p-8 shadow-2xl relative overflow-hidden">
      ...
      <div className="relative z-10 flex flex-col md:flex-row justify-between items-center mb-6 sm:mb-8 gap-4">
        <div className="flex items-center gap-4 w-full sm:w-auto justify-between sm:justify-start">
            <h2 className="text-xl sm:text-3xl font-bold text-slate-100 tracking-tight">{monthName}</h2>
        ...
        <div className="flex items-center bg-slate-800/50 rounded-xl p-1 border border-slate-700/50 w-full sm:w-auto justify-between sm:justify-end">
          <button onClick={() => navigateMonth(-1)} className="p-2 hover:bg-slate-700 rounded-lg text-slate-400 hover:text-white transition"><ChevronLeft size={20} /></button>
          <button onClick={jumpToToday} className="px-4 py-2 text-sm font-medium text-slate-300 hover:text-white transition border-x border-slate-700 mx-1">Today</button>
          <button onClick={() => navigateMonth(1)} className="p-2 hover:bg-slate-700 rounded-lg text-slate-400 hover:text-white transition"><ChevronRight size={20} /></button>
        </div>
      </div>
